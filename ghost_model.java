...
public abstract class ghost_model {
    ...
    public EnumSet.Direction frightened(int ghostX, int ghostY, EnumSet.Direction direction,Pacman pacman) {
        switch (PacmanGame.MapData[ghostX + 20 * (ghostY - 1) - 1] & 15) {        //判斷鬼該面朝哪
            case 1:                    //上有牆壁
                if (direction == EnumSet.Direction.up) {                //鬼從下上來
                    if ((ghostX - pacman.gridX) >= 0) {        //鬼在人右邊
            ...
            case 0:                    //十字路口
                if (direction == EnumSet.Direction.left || direction == EnumSet.Direction.right) {                //面對左或右
                    if ((ghostY - pacman.gridY) <= 0) {        //鬼在人上面
                        direction = EnumSet.Direction.up;
                    } else if ((ghostY - pacman.gridY) >= 0) {    //鬼在人下面
                        direction = EnumSet.Direction.down;
                    }
                } else {                                    //面對上或下
                    if ((ghostX - pacman.gridX) <= 0) {        //鬼在人左邊
                        direction = EnumSet.Direction.left;
                    } else if ((ghostX - pacman.gridX) >= 0) {    //鬼在人右邊
                        direction = EnumSet.Direction.right;
                    }
                }
    ...
    protected abstract void Eaten(Pacman pacman);



    public abstract void Hit(Pacman pacman);


    public abstract EnumSet.Direction GetDirection(Pacman pacman);
